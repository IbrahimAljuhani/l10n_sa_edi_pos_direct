...
class PosOrder(models.Model):
    ...
    def submit_to_zatca_reporting(self):
        ...
        try:
            ...
            # Check for "already submitted" case (409 - concurrent cron runs submitting
            # the same invoice). Per ZATCA reporting API docs, the 409 body has no top-level
            # 'error' key - only {'validationResults': {...}, 'reportingStatus': 'NOT_REPORTED',
            # 'status_code': 409}, with the message inside validationResults.errorMessages, e.g.
            ...
            if result.get('status_code') == 409:
                ...
                already_reported = any(
                    'already' in (m.get('message') or '').lower()
                    and 'reported' in (m.get('message') or '').lower()
